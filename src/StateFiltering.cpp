    ...
    const std::array<bool,N_FEET>& contact)
{
    ...
    // ZUPT disabled: harmful during walking (double-support phase has non-zero CoM velocity).
    // The dual FK constraints during double support already constrain position.
    // Re-enable conditionally for static standing if velocity drift becomes an issue.
