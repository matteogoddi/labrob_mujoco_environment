...
std::shared_ptr<WholeBodyController>
WalkingManager:: getWholeBodyControllerPointer(){
    return whole_body_controller_ptr_;
}
...
WalkingManager::update(
    ...
    eta2 = std::pow(ismpc_ptr_->getOmega(), 2.0);
    ...
    if (isEKFactive && t_msec_ >= startTimeEKF) {
        ...
        base_estimation_robot_state.linear_velocity = go_base_velocity;
        base_estimation_robot_state.angular_velocity = actual_output.segment(njnt + 3 + 3, 3);
        ...
        // go_imu_quaternion = q_yaw * go_imu_quaternion;
    ...
    if(!useRobot){
        ...
        actual_output.segment(3,3) = rotVecFromQuaternion((Eigen::Quaterniond) sim_robot_data.oMf[imu_idx_].rotation());
    ...
    #pragma omp parallel sections num_threads(2)
    {
        #pragma omp section
        {
            if(t_msec_ >= startTimeEKF && isEKFactive) {
                ...
                // fb_robot_state.position = go_base_position;
                // fb_robot_state.linear_velocity = go_base_velocity;
                // fb_robot_state.angular_velocity = actual_output.segment(njnt + 3, 3);
                ...
                //     fb_robot_state.joint_state[joint_name].vel = actual_output(njnt + joint_id + 3 + 3);
                // }
            }
            else{
    ...
    //add sinusoial movement to lip state
    ...
    if (t_msec_ >= 10000){
        p_CoM_des.y() += 0.05/5 * std::sin(M_PI * 0.5 * (t_msec_ - 10000)/ 1000);
        v_CoM_des.y() += 0.05/5 *  0.5 * M_PI /1000* std::cos(M_PI * 0.5 * (t_msec_ - 10000)/ 1000);
        p_ZMP_des.y() += 0.05/5 * std::sin(M_PI * 0.5 * (t_msec_ - 10000)/ 1000);
    } else {
