...
ISMPC_dist::solve(
  ...
  b_eq_ << com_pos(0) + state.com_vel_(0) / eta_ - zmp_pos(0) + disturbance.x()/std::pow(eta_, 2.0),
      com_pos(1) + state.com_vel_(1) / eta_ - zmp_pos(1) + disturbance.y()/std::pow(eta_, 2.0),
      com_pos(2) + state.com_vel_(2) / eta_ - zmp_pos(2) + disturbance.z() / std::pow(eta_, 2.0);
...
const Eigen::Vector3d& ISMPC_dist::getInput() const {
  return input_;
}

const Eigen::VectorXd& ISMPC_dist::getInputSequenceX() const {
  return zDotOptimalX;
}
const Eigen::VectorXd& ISMPC_dist::getInputSequenceY() const {
  return zDotOptimalY;
}
const Eigen::VectorXd& ISMPC_dist::getInputSequenceZ() const {
  return zDotOptimalZ;
}

//return A_eq^-1*b_eq
Eigen::Vector3d
ISMPC_dist::getStabConstraintOffset() const {
  //use ldlt
...
double
ISMPC_dist::getEta() const {
  return eta_;
}

void
ISMPC_dist::setEta(double eta) {
  eta_ = eta;
}

double
ISMPC_dist::clamp(double n, double n_min, double n_max) {
