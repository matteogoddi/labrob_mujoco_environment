...
static std::atomic<ControlMode> ctrl_mode{SECURITY_STOP};
    ...
    switch (ctrl_mode) {
      ...
      case REGULATION: {
        ...
        for (int i = 0; i < G1_NUM_MOTOR; ++i) {
          ...
          motor_command_tmp.dq_target.at(i) = 0.0;
          motor_command_tmp.kp.at(i) = Kp_reg[i];
      ...
      case WBC: {
        ...
        // // ------ fill robot state -------
        // robot_state.position = filtered_state.segment<3>(0);     // RIVEDI
        // robot_state.orientation.coeffs() = filter_params.segment<4>(0);
        // robot_state.linear_velocity = robot_state.orientation.toRotationMatrix().transpose() * filtered_state.segment<3>(3);
        // robot_state.angular_velocity = filter_input.segment<3>(3);

        // for (int i = 0; i < G1_NUM_MOTOR; ++i) {
        //   const std::string& name = robot_model_.names[joint_id];   // RIVEDI
        //   robot_state.joint_state[name].pos = ms->q.at(i); 
        //   robot_state.joint_state[name].vel = ms->dq.at(i); 
        // }
        ...
        //   motor_command_tmp.kd.at(i) = KdwbcKp_wbc[i];
        // }
...
int main(int argc, char const *argv[]) {
  if (argc < 2) {
    std::cout << "Usage: g1_conttroller network_interface" << std::endl;
    exit(0);
  }
